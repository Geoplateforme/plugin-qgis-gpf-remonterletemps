...
1. From the command-line (a terminal with qgis executable in `PATH` or OSGeo4W Shell):
    ...
    ```sh
    # Linux
    qgis --profile plg_gpf_remonter_le_temps
    # Windows - OSGeo4W Shell
    qgis-ltr --profile plg_gpf_remonter_le_temps
    # Windows - PowerShell opened in the QGIS installation directory
    PS C:\Program Files\QGIS 3.40.4\LTR\bin> .\qgis-ltr-bin.exe --profile plg_gpf_remonter_le_temps
    ```
